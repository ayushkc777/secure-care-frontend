...
export function ResendVerificationPage() {
  ...
  const submit = handleSubmit(async (values) => {
    ...
    try {
      await postWithCsrf("/auth/email/resend", values);
      setCompleted(true);
    } catch (error) {
  ...
  return (
    <AuthCard title={completed ? "Check your email" : "Resend verification"}>
      {completed ? (
        <>
          <p role="status">
            If the account is awaiting verification, new instructions will be provided.
          </p>
        ...
        <>
          <p>Request a replacement for an expired or previously issued verification link.</p>
          ...
          <form className="auth-form" noValidate onSubmit={(event) => void submit(event)}>
            ...
            <button className="primary-button" disabled={isSubmitting} type="submit">
              {isSubmitting ? "Requesting link…" : "Send verification link"}
            </button>
          </form>
        </>
      )}
